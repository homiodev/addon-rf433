...
public class Rf433Entrypoint implements BundleEntrypoint {
  ...
  public RF433Signal readSignal(int rf433ReceiverPinAddress, Rf433JSON rf433JSON) throws IOException, InterruptedException {
    ...
    if (!isTestApplication) {
      ...
      FileChannel fileChannel =
          new RandomAccessFile(rf433Dir.resolve("Rf433SnifferResult.times").toFile(), "r").getChannel();
  ...
  public List<String> buildAndRunScriptFromSignal(RF433Signal signal, Rf433JSON rf433JSON, int rf433Pin)
  ...
  public static class RF433Signal implements Serializable {
    ...
    public Signal[] findDuplicates(float maxTime) {
      ...
      while (firstBucket != null) {
        ...
        Pair<Integer, Integer> secondBucket = findTimeBucketStartFrom(fromIndex, maxTime,
            blockedIndexes); // second bucket stars from end of first bucket
    ...
    private void assertSize() {
      if (times.length != values.length) {
        throw new RuntimeException(
            String.format("Times length %d must be same as values length %d", times.length, values.length));
      }
    }
